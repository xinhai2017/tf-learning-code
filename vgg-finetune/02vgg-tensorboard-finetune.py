...
fc1 = tf.layers.dense(flatten, 256)
fc2 = tf.layers.dense(fc1, 128)
fc3 = tf.layers.dense(fc2, 64, trainable=False)
logits = tf.layers.dense(fc3,10)
...
with tf.Session() as sess:
    ...
    for i in range(train_steps):
        ...
        if (i+1) % output_model_every_steps == 0:
            saver.save(sess, os.path.join(model_dir,'vgg-ckpt-%05d' % (i+1)))
            # print('model saved to ckpt-%05d' % (i+1))