...
def convnet(inputs, activation, kernel_initializer):
    conv1_1 = tf.layers.conv2d(inputs,
                               ...
                               # 默认初始化方法：tf.glorot_uniform_initializer
                               name="conv1_1")
    ...
    return ((conv1_1,conv1_2,conv2_1,conv2_2,conv3_1,conv3_2),
            (flatten))
#relu train 10k : 73.35%  vs sigmoid: 53.39%
#默认使用初始化：tf.glorot_uniform_initializer：76.53% 100k train.
#tf.truncated_normal_initializer 78.04 100k train
#tf.keras.initializers.he_normal: 71.52% 100k train
conv_list, flatten = convnet(x_image, tf.nn.relu, tf.truncated_normal_initializer(stddev=0.02))
# conv_list, flatten = convnet(x_image, tf.nn.sigmoid)
conv1_1,conv1_2,conv2_1,conv2_2,conv3_1,conv3_2 = conv_list
...
with tf.name_scope("train_op"):
    ...
    # train_op = tf.train.GradientDescentOptimizer(1e-4).minimize(loss) # 12.35% train 10k
    #momentum: #35.75 train 10k
